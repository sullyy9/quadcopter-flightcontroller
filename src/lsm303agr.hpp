...
#define ADDRESS_ACCEL 0b00110010
#define ADDRESS_MAG 0b00111100
...
#define ACCEL_CTRL_REG_1_VAL                                                       \
    ((ACCEL_DATA_RATE << 4) + (ACCEL_LOW_POWER_ON << 3) + (ACCEL_Z_AXIS_ON << 2) + \
     (ACCEL_Y_AXIS_ON << 1) + (ACCEL_X_AXIS_ON << 0))
...
#define ACCEL_CTRL_REG_2_VAL                                                                   \
    ((ACCEL_HP_FILTER_MODE << 6) + (ACCEL_HP_CUTOFF_FREQ << 4) + (ACCEL_FILTER_DATA_ON << 3) + \
     (ACCEL_FILTER_CLICK_ON << 2) + (ACCEL_FILTER_INT1_ON << 1) + (ACCEL_FILTER_INT2_ON << 0))
...
#define ACCEL_CTRL_REG_3_VAL                                                              \
    ((ACCEL_INT1_CLICK_ON << 7) + (ACCEL_INT1_AOI1_ON << 6) + (ACCEL_INT1_AOI2_ON << 5) + \
     (ACCEL_INT1_DRDY1_ON << 4) + (ACCEL_INT1_DRDY2_ON << 3) + (ACCEL_INT1_WTM_ON << 2) + \
...
#define ACCEL_CTRL_REG_4_VAL                                                                \
    ((ACCEL_BLOCK_UPDATE << 7) + (ACCEL_LITTLE_BIG_ENDIAN << 6) + (ACCEL_MEAS_RANGE << 4) + \
     (ACCEL_HIGH_RES_ON << 3) + (ACCEL_SELF_TEST << 1) + (ACCEL_SPI_3_WIRE_ON << 0))
...
#define ACCEL_CTRL_REG_5_VAL                                                                    \
    ((ACCEL_REBOOT_MEM << 7) + (ACCEL_FIFO_ON << 6) + (ACR5_UNUSED << 4) +                      \
     (ACCEL_INT1_LATCH_ON << 3) + (ACCEL_INT1_4D_DETECT_ON << 2) + (ACCEL_INT2_LATCH_ON << 1) + \
...
#define ACCEL_CTRL_REG_6_VAL                                                                      \
    ((ACCEL_INT2_CLICK_ON << 7) + (ACCEL_INT2_INT1_FUNC_ON << 6) +                                \
     (ACCEL_INT2_INT2_FUNC_ON << 5) + (ACCEL_INT2_BOOT_ON << 4) + (ACCEL_INT2_ACTIVITY_ON << 3) + \
     (ACR5_UNUSED_1 << 2) + (ACCEL_INT_ACTIVE_H_L << 1) + (ACR5_UNUSED_2 << 0))
...
#define MAG_CONFIG_REG_A_VAL                                                   \
    ((MAG_TEMP_COMP_ON << 7) + (MAG_REBOOT_MEM << 6) + (MAG_SOFT_RESET << 5) + \
     (MAG_LOW_POWER_ON << 4) + (MAG_DATA_RATE << 2) + (MAG_MODE_SELECT << 0))
...
#define MAG_CONFIG_REG_B_VAL                                                                 \
    ((MCRB_UNUSED_1 << 7) + (MCRB_UNUSED_2 << 6) + (MCRB_UNUSED_3 << 5) +                    \
     (MAG_OFFSET_SINGLE_MODE << 4) + (MAG_INT_CORRECTION << 3) + (MAG_SET_PULSE_FREQ << 2) + \
...
#define MAG_CONFIG_REG_C_VAL                                                              \
    ((MCRC_UNUSED_1 << 7) + (MAG_INT_MAG_PIN_ON << 6) + (MAG_I2C_OFF << 5) +              \
     (MAG_BLOCK_DATA_UPDATE << 4) + (MAG_BIG_LITTLE_ENDIAN << 3) + (MCRC_UNUSED_2 << 2) + \
...
#define MAG_INT_REG_VAL                                                                       \
    ((MAG_INT_X_ON << 7) + (MAG_INT_Y_ON << 6) + (MAG_INT_Z_ON << 5) + (MICR_UNUSED_1 << 4) + \
     (MICR_UNUSED_2 << 3) + (MAG_INT_ACTIVE_LOW_HIGH << 2) + (MAG_INT_LATCH_ON << 1) +        \
     (MAG_INT_ON << 0))
